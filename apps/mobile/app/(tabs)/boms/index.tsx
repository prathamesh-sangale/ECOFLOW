...
const fetchBOMs = async ({ pageParam = 1, queryKey }: any) => {
...
export default function BOMsScreen() {
  ...
  const boms = data ? data.pages.flatMap((page) => page.data) : [];

  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity 
      style={styles.card} 
      onPress={() => router.push(`/(tabs)/boms/${item.id}`)}
      activeOpacity={0.7}
    >
      <View style={styles.cardHeader}>
        ...
        <View style={styles.cardTitleContainer}>
          <Text style={styles.cardTitle}>{item.bom_number}</Text>
      ...
      <View style={styles.cardBody}>
        <View style={styles.infoRow}>
          <SymbolView name="cube.box" size={14} tintColor="#94A3B8" />
          <Text style={styles.infoText} numberOfLines={1}>{item.product?.product_name || 'No Product'}</Text>
        </View>
        <View style={styles.infoRow}>
  ...
  return (
    <View style={styles.container}>
      ...
      {status === 'pending' ? (
        ...
        <View style={styles.centered}><Text style={styles.errorText}>Error fetching BOMs: {(error as any).message}</Text></View>
      ) : (
        <FlatList
          data={boms}
